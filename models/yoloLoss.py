...
class Yolo_loss(nn.Module):
    ...
    def forward(self, xin, labels=None):
        ...
        for output_id, output in enumerate(xin):
            ...
            pred = output[..., :4].clone()
            pred[..., 0] += self.grid_x[output_id][:batchsize]
            pred[..., 1] += self.grid_y[output_id][:batchsize]
            pred[..., 2] = torch.exp(pred[..., 2]) * self.anchor_w[output_id][:batchsize]
            pred[..., 3] = torch.exp(pred[..., 3]) * self.anchor_h[output_id][:batchsize]
