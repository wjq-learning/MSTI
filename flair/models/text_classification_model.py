...
class TextClassifier(flair.nn.Model):
    ...
    The model takes word embeddings, puts them into an RNN to obtain a Textual target labels representation, and puts the
    Textual target labels representation in the end into a linear layer to get the actual class label.
