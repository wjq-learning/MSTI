...
class TextDataset(Dataset):
    ...
    def charsplit(
        ...
        """Tokenizes a Textual target labels file on character basis."""
        assert path.exists()

        lines = open(path, "r", encoding="utf-8").readlines()
        log.info(f"read Textual target labels file with {len(lines)} lines")
    ...
    def tokenize(self, path: Path):
        """Tokenizes a Textual target labels file."""
        assert path.exists()
