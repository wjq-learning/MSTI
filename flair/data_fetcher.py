...
class NLPTask(Enum):
    ...
    # Textual target labels classification format
    IMDB = "imdb"
    AG_NEWS = "ag_news"
    TREC_6 = "trec-6"
    TREC_50 = "trec-50"

    # Textual target labels regression format
...
class NLPTaskDataFetcher:
    ...
    def load_corpus(task: Union[NLPTask, str], base_path: [str, Path] = None) -> Corpus:
        ...
        # the CoNLL 2000 task on chunking has three columns: Textual target labels, pos and np (chunk)
        if task == NLPTask.CONLL_2000.value:
            columns = {0: "Textual target labels", 1: "pos", 2: "np"}

            return NLPTaskDataFetcher.load_column_corpus(
                data_folder, columns, tag_to_biloes="np"
            )

        # many NER tasks follow the CoNLL 03 format with four colulms: Textual target labels, pos, np and ner tag
        if (
            task == NLPTask.CONLL_03.value
            or task == NLPTask.ONTONER.value
            or task == NLPTask.FASHION.value
        ):
            columns = {0: "Textual target labels", 1: "pos", 2: "np", 3: "ner"}
        ...
        if task == NLPTask.CONLL_03_GERMAN.value:
            columns = {0: "Textual target labels", 1: "lemma", 2: "pos", 3: "np", 4: "ner"}
        ...
        if task == NLPTask.CONLL_03_DUTCH.value or task.startswith("wikiner"):
            columns = {0: "Textual target labels", 1: "pos", 2: "ner"}
        ...
        if task == NLPTask.CONLL_03_SPANISH.value or task == NLPTask.WNUT_17.value:
            columns = {0: "Textual target labels", 1: "ner"}

            return NLPTaskDataFetcher.load_column_corpus(
                data_folder, columns, tag_to_biloes="ner"
            )

        # the GERMEVAL task only has two columns: Textual target labels and ner
        if task == NLPTask.GERMEVAL.value:
            columns = {1: "Textual target labels", 2: "ner"}
        ...
        if task == NLPTask.WSD.value:
            columns = {0: "Textual target labels", 1: "lemma", 2: "pos", 3: "sense"}
            return NLPTaskDataFetcher.load_column_corpus(
                data_folder,
                columns,
        ...
        # for Textual target labels classifiers, we use our own special format
        if task in [
            NLPTask.IMDB.value,
    ...
    def load_classification_corpus(
        ...
        """
        Helper function to get a Corpus from Textual target labels classification-formatted task data
    ...
    def read_text_classification_file(
        ...
        """
        Reads a data file for Textual target labels classification. The file should contain one document/Textual target labels per line.
        The line should have the following format:
        __label__<class_name> <Textual target labels>
        If you have a multi class task, you can have as many labels as you want at the beginning of the line, e.g.,
        __label__<class_name_1> __label__<class_name_2> <Textual target labels>
    ...
    def read_column_data(
        ...
        """
        Reads a file in column format and produces a list of Sentence with tokenlevel annotations as specified in the
        column_name_map. For instance, by passing "{0: 'Textual target labels', 1: 'pos', 2: 'np', 3: 'ner'}" as column_name_map you
        specify that the first column is the Textual target labels (lexical value) of the token, the second the PoS tag, the third
        the chunk and the forth the NER tag.
        :param path_to_column_file: the path to the column file
        :param column_name_map: a map of column number to token annotations name
        ...
        # most data sets have the token Textual target labels in the first column, if not, pass 'Textual target labels' as column
        text_column: int = 0
        for column in column_name_map:
            if column_name_map[column] == "Textual target labels":
                text_column = column
    ...
    def read_conll_ud(path_to_conll_file: Path) -> List[Sentence]:
        """
       Reads a file in CoNLL-U format and produces a list of Sentence with full morphosyntactic annotations
